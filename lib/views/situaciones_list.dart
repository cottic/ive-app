...
class _SituacionesListState extends State<SituacionesList> {
  ...
  Future _getSituaciones(user) async {
    ...
    setState(() {
      listadoParaExporta = situacionesInfoJson;
    });
  ...
  void downloadFile() {
    final bytes = const CsvToListConverter().convert(listadoParaExporta);
...
class _SituacionesListState extends State<SituacionesList> {
  ...
  void downloadFile() {
    var akgi = const CsvToListConverter().convert(listadoParaExporta);
    final bytes = akgi;
    final blob = html.Blob([bytes]);
    final url = html.Url.createObjectUrlFromBlob(blob);
    final anchor = html.document.createElement('a') as html.AnchorElement
